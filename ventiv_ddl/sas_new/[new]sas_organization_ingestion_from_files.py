...
table_name = "organization" # Update table name here as neccessary, accepted values: claims, exposures, organization, policy, and premium
...
df_import_02 = spark.sql(f"""
    SELECT '{as_of_date}' AS as_of,
        a.*,
        CAST(SUBSTRING(a.org_last_updated_date_char, 1, 9) AS DATE) AS organization_last_update_date,
        CAST(SUBSTRING(a.organization_created_date_char, 1, 9) AS DATE) AS organization_created_date
