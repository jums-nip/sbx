...
# # Step n; Read schema from existing Delta table
# target_delta_table = spark.read.format("delta").load("/user/hive/warehouse/delta_nip.db/claims")
...
# df_import_02_schema = spark.createDataFrame(df_import_02.rdd, schema=existing_table_schema) # Error
# df_import_02_schema.display()

# COMMAND ----------

# Create a select expression that matches each column in the target schema
from pyspark.sql.functions import col, lit

# Get column lists for both DataFrames
target_columns = set(target_delta_table.columns)
joined_columns = set(df_import_02.columns)

# Build select expressions that match the target schema
select_expressions = []

for field in target_delta_table.schema.fields:
...
df_import_02_with_schema = df_import_02.select(*select_expressions)
...
if verify_schema_match:
    df_import_02_with_schema.write.format("delta").mode("append").saveAsTable("delta_nip.claims")
